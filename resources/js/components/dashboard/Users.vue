<template>
    <div class="container w-full md:w-4/5 xl:w-4/5 mx-auto px-2 my-3">
        <div class="sm:flex items-center justify-between m-9 mb-0">
            <h1 class="font-bold text-gray-600 text-4xl">Users List</h1>
            <button
                class="bg-gray-800 rounded-lg hover:bg-gray-700 focus:outline-none focus:ring focus:ring-gray-300 focus:ring-opacity-50 text-white py-2 px-4 hover:scale-105 duration-300 my-3 sm:mt-0 sm:w-auto w-full"
                @click="toggleModal(), setHeading('Add New Admin')"
            >
                Add New Admin
            </button>
        </div>
        <p class="mt-3 text-sm text-gray-600 m-9 hidden sm:block">
    ...
    <Modal @close="toggleModal" :modalActive="modalActive" :heading="heading">
        ...
        <form class="my-8 px-4" @submit.prevent="register" v-else>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                <input
                    class="block w-full px-4 py-2 my-2 text-gray-700 bg-white border rounded-lg focus:border-blue-400 focus:ring-opacity-40 focus:outline-none focus:ring focus:ring-blue-300 placeholder-gray-600"
                    type="email"
                    name="email"
                    id="email"
                    v-model="form.email"
...
export default {
    ...
    setup() {
        ...
        Echo.private("user-channel").listen("UserEvent", (e) => {
            options.value = e;
            paginateOptions();
        });
        const toggleModal = () => {
        ...
        const register = async () => {
            ...
            let url = "";
            if (heading.value === "Add New Admin") {
                url = "register";
            } else {
                url = "edit";
            }
